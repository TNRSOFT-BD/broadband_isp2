...
import { CheckCircle2, Plus, Pencil, Trash2, Shield } from 'lucide-react';
import { useState } from 'react';
import { useAdminUrl } from '@/hooks/use-admin-url';


const ICON_OPTIONS = [
    'Shield', 'Heart', 'Zap', 'BookOpen', 'Award', 'Globe', 'Users', 'Eye',
    'Target', 'Clock', 'Headphones', 'CheckCircle2', 'Star', 'Lock', 'Server', 'Wifi',
];

interface CoreValueItem { id: number; icon: string; title: string; description: string; sort_order: number; is_active: boolean; }
interface PageProps extends Record<string, unknown> { coreValues: CoreValueItem[]; }
...
export default function CoreValuesIndex() {
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: 'Core Values', href: adminUrl('/pages/about/core-values') },
    ];

    const { coreValues } = usePage<PageProps>().props;
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (editingId) {
            form.put(route('admin.about-us.core-values.update', editingId), { onSuccess: () => { setShowForm(false); setEditingId(null); form.reset(); } });
        } else {
            form.post(route('admin.about-us.core-values.store'), { onSuccess: () => { setShowForm(false); form.reset(); } });
        }
    };

    const handleEdit = (item: CoreValueItem) => {
    ...
    const handleDelete = (id: number) => { if (confirm('Are you sure?')) router.delete(route('admin.about-us.core-values.destroy', id)); };
    const handleToggleStatus = (id: number) => { router.patch(route('admin.about-us.core-values.toggle-status', id)); };
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
                <div className="flex flex-wrap items-center justify-between gap-4">
                    <div><h1 className="text-2xl font-bold tracking-tight">Core Values</h1><p className="mt-1 text-sm text-muted-foreground">Manage company principles and values.</p></div>
                    <Button onClick={() => { setShowForm(!showForm); setEditingId(null); form.reset(); }}><Plus className="mr-1 h-4 w-4" /> Add Core Value</Button>
                </div>
                ...
                {showForm && (
                    <Card>
                        <CardHeader><CardTitle>{editingId ? 'Edit' : 'Create'} Core Value</CardTitle></CardHeader>
                        <CardContent>
                            <form onSubmit={handleSubmit} className="grid gap-4 sm:grid-cols-2">
                                <div><Label>Title *</Label><Input value={form.data.title} onChange={(e) => form.setData('title', e.target.value)} placeholder="Reliability" className={inputCls} required /></div>
                ...
                <Card>
                    <CardHeader><CardTitle>All Core Values ({coreValues.length})</CardTitle></CardHeader>
                    <CardContent>
                        {coreValues.length === 0 ? (<div className="py-12 text-center text-gray-500"><Shield className="mx-auto h-12 w-12 text-gray-300" /><p className="mt-4 text-sm">No core values created yet.</p></div>) : (
